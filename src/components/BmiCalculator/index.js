...
const BmiCalculator = () => {
  const heightObject = JSON.parse(localStorage.getItem('height'))
  const weightObject = JSON.parse(localStorage.getItem('weight'))

  const [height, setHeight] = useState(
    heightObject !== null ? heightObject : 170,
  )
  const [weight, setWeight] = useState(
    weightObject !== null ? weightObject : 60,
  )
